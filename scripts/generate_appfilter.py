...
APPFILTER_FILE = environ.get(
    'APPFILTER_FILE',
    path.join(ABS_PATH, '../app/src/main/res/xml/appfilter.xml'),
)

makedirs(path.dirname(path.abspath(APPFILTER_FILE)), exist_ok=True)

if path.exists(APPFILTER_FILE):
    remove(APPFILTER_FILE)

with open(DB_FILE, 'r', encoding='utf-8') as db_obj:
    try:
...
app_filter_content += '</resources>'
with open(APPFILTER_FILE, 'w', encoding='utf-8') as app_filter_obj:
